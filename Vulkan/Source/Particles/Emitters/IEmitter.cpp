...
IEmitter::IEmitter(const uint32_t& tech_id, const uint32_t& buffer_size)
    : m_TechId(tech_id)
...
void IEmitter::Emit(const uint32_t& count_per_second)
{
...
CParticleManager* IEmitter::ParticleMgr()
{
    return g_Engine->ParticleMgr();
}

physx::PxParticleSystem* IEmitter::CreateParticleSystem()
{
...
void IEmitter::PrepareParticleSystem(physx::PxParticleSystem* ps)
{
    ps->setGridSize(3.0f);
...
void IEmitter::CreateParticles(std::vector<uint32_t>& idx_out, std::vector<physx::PxVec3>& dir_out, const uint32_t& emit_count)
{
...
void IEmitter::ReleaseParticles(const std::vector<uint32_t>& index_buff)
{
...
void IEmitter::MakeStepInParticleLife(const uint32_t& cpu_idx)
{
    auto& data = ParticleMgr()->BuffData();
    data.Particles[cpu_idx].life -= 0.001f;
}

void IEmitter::UpdateParticlePos(const uint32_t& cpu_idx, const physx::PxVec3& new_pos)
{
    auto& data = ParticleMgr()->BuffData();
...
const uint32_t& IEmitter::ReserveSlotInBuffer()
{
    auto& data = ParticleMgr()->BuffData();
...
void IEmitter::SendParticlesToPhysX(const std::vector<uint32_t>& idx, const std::vector<physx::PxVec3>& dir)
{
...
uint32_t IEmitter::LowestBitSet(uint32_t x)
{
    if (x == 0)
        return 0;
...
void IEmitter::GenerateRandomXYZ(float& x, float& y, float& z)
{
